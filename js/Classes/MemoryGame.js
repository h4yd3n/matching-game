class MemoryGame {
    ...
    playGame() {
        ...
        // only use fraction of the arr for that level of play
        // get the game pairs from the value of the select obj
        this.totPieces = document.getElementById('chooser').value
    
        if(this.totPieces == 0) { // unchosen menu value == 0
            this.totPieces = 12 // no choice is Novice level
        }
        ...
        // splice off the portion of the array needed for game
        this.splicedArr = this.arr.splice(0, this.totPieces)

        // double the array (matching games need pairs)
        //this.copyArr = this.arr.slice() // back up the array
        this.gameArr = [...this.splicedArr, ...this.splicedArr]
       
        //alert(this.gameArr)
        // randomize the array items
        this.gameArr.sort((a, b) => {
        ...
        if(this.picChoices.length == 2) {
            ...
            if(this.picChoices[0].name == this.picChoices[1].name) {
                ...
                if(this.picChoices[0].id != this.picChoices[1].id) {
                    ...
                    let picChoice0 = document.getElementById(this.picChoices[0].id) // get clicked img choices
                    let newPic0 = new Image()
                    newPic0.src = `images/final/200x200${this.picChoices[0].name}.jpg`
                    picChoice0.replaceChild('click', this.showPic.bind(this))

                    // prevent re-clicking the sa
                    let picChoice1 = document.getElementById(this.picChoices[1].id) // get clicked img choices
                    let newPic1 = new Image()
                    newPic1.src = `images/final/200x200${this.picChoices[1].name}.jpg`
                    picChoice1.replaceChild('click', this.showPic.bind(this))
            ...
            this.score = Math.ceil(this.average * this.gameArr.length * this.gameArr.length * this.matches * 100 / this.totSec) 
